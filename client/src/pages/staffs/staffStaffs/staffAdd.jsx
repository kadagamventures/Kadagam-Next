...
import { addStaff, updateStaff } from "../../../redux/slices/staffSlice";
...
const AddStaff = () => {
  ...
  const { items: staffList, rawPassword } = useSelector((state) => state.staff);
  ...
  const [modalContent, setModalContent] = useState(null); // file URL or blob
  const [modalTitle, setModalTitle] = useState("");

  // Keys for resetting file inputs
  const [profilePicKey, setProfilePicKey] = useState(Date.now());
  const [resumeKey, setResumeKey] = useState(Date.now());

  // Staff ID validation state
  ...
  }, [isEditMode, id, staffList]);

  // Debounced validation for staff ID uniqueness
  const validateStaffId = useCallback(
    debounce(async (staffId) => {
      ...
      try {
        ...
        const res = await axios.get(`${API_BASE_URL}/check-id?staffId=${staffId}`, {
  ...
  // Open modal to view a file (image or document)
  const handleViewFile = (file, title) => {
    if (!file) return;
    setModalTitle(title);
    setModalContent(file.preview || file);
    setModalOpen(true);
  };

  // Delete file and reset file input by updating key state
  ...
  const handleAddStaff = async () => {
    ...
    let parsedAssignedProjects = newStaff.assignedProjects;
    let parsedPermissions = newStaff.permissions;
    if (typeof parsedAssignedProjects === "string") {
      try {
        parsedAssignedProjects = JSON.parse(parsedAssignedProjects);
      } catch (err) {
        parsedAssignedProjects = [];
      }
    }
    ...
    try {
      ...
      formData.append("assignedProjects", JSON.stringify(parsedAssignedProjects));
      formData.append("permissions", JSON.stringify(parsedPermissions));
      if (newStaff.profilePic) {
      ...
      const resultAction = await dispatch(
        isEditMode
          ? updateStaff({ id: newStaff._id, formData })
          : addStaff(formData)
      );
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-4 rounded shadow-lg max-w-lg w-full relative">
            ...
            {modalContent && modalTitle === "Profile Picture" ? (
              <img src={modalContent} alt="Profile Preview" className="max-h-96 mx-auto" />
            ) : (
              <a
                href={modalContent}
                target="_blank"
            ...
            <div className="space-y-4">
              ...
              <div className="mb-4">
                ...
                {newStaff.profilePic && (
                  <div className="flex items-center mt-2 space-x-4">
                    <button
                      onClick={() =>
                        handleViewFile(newStaff.profilePic, "Profile Picture")
                      }
              ...
              <div>
                ...
                {newStaff.resume && (
                  <div className="flex items-center mt-2 space-x-4">
                    <button
                      onClick={() => handleViewFile(newStaff.resume, "Resume")}
                      className="px-3 py-1 bg-blue-500 text-white rounded"
                    >
                      View
                    </button>
